...
- **Modelo Ella** — **persona-tipo prompt-only** (S1.3 / ADR-0015) que aparece como "modelo" recorrente em **toda peça** do catálogo (Foto 3, ADR-0008). **Não confundir com Ellen Lopes.** Ellen é a pessoa real fundadora; Modelo Ella é a persona-tipo da cliente-arquétipo descrita por prompt. **Cada Foto 3 do catálogo pode mostrar uma mulher diferente** dentro da persona-tipo: mulher 45–50 anos, brasileira morena warm tan, cabelo escuro médio-longo natural, elegante de luxo discreto, expressão serena, manicure neutra, iluminação warm golden hour ou indoor janela. **Não é** jovem 20–30, **não é** loira, **não é** corporativa fria. **Não é** Soul Character treinado — geração via Nano Banana Pro 2K com prompt master independente cada vez. Persona materializada em `assets/prompts/personas/modelo-ella-persona-tipo.md` (master) + `assets/prompts/personas/sub-prompts/{mao,pescoco,orelha,tornozelo}.md` (variation hooks por categoria). Drift de **rosto** entre peças é OK e desejado — o que se mantém é a **estética** (idade, etnia, mood, styling). ADR-0012 (Soul Character único) superseded por ADR-0015.
- **Identidade Atemporal** / **BIOS Visual** — conjunto fechado de regras visuais permanentes da Marca. Paleta primária pixel-amostrada (S1.1 / ADR-0003 atualizada): rosa salmão `#FFD9CC` + dourado mostarda `#D99A30` + preto warm `#251008`. Paleta secundária warm derivada (S1.2 / ADR-0014): salmão-claro `#FFF1ED` + areia `#F0DCC4` + taupe `#8A6E5C` + dourado-claro `#EFC78B`. Tipografia hero **Bodoni Moda** (Google Fonts, weight adaptativo 500 mobile / 400 desktop — S1.2 / ADR-0014). Tipografia secundária **Inter** (Google Fonts, S1.2 / ADR-0014). Motif sparkles dourados; "perna alongada do A" como assinatura sutil presente apenas no logo bitmap. Materializado em `assets/prompts/brand-reference.md` v1.0. Mudança nesta camada exige nova ADR superando ADR-0014 (e ADR-0003 quando logo for vetorizada).
...
- **Camada Atemporal da Marca** — produzida uma vez, reutilizada para sempre. Brand Reference Pack, Persona-Tipo Modelo Ella (prompt master + sub-prompts), logo derivados, sparkles SVG, hero da home (Marca), fotos institucionais, favicon, OG image padrão. Mudar exige ADR superando ADR-0003.
...
- **Persona-Tipo** (substitui o conceito anterior de Soul Character — S1.3 / ADR-0015) — descrição estruturada por prompt de uma persona recorrente. Cada geração via Nano Banana Pro 2K roda independente (sem `reference_id`). Modelo Ella é a persona-tipo atemporal da Marca em `assets/prompts/personas/modelo-ella-persona-tipo.md`. Coerência entre fotos vem da fidelidade ao prompt master, não da consistência de rosto.
...
| Drift | Inconsistência da **estética** da persona-tipo entre páginas (idade, etnia, mood, styling fora do range). Proibido. **Drift de rosto é OK** desde S1.3 / ADR-0015 — cada Foto 3 pode mostrar mulher diferente dentro da persona-tipo. |
| BIOS Visual | Camada Atemporal da Marca. Imutável sem ADR. |
| Brand Reference Pack | `assets/prompts/brand-reference.md`. Versão atual v1.0 (S1.2 / ADR-0014). Codifica BIOS Visual + templates de prompt Higgsfield. Input obrigatório de toda geração visual. Mudança exige ADR superando ADR-0014. |
| Paleta secundária | 4 cores warm derivadas da primária — salmão-claro, areia, taupe, dourado-claro. Tokens em `app/globals.css` `@theme`. S1.2 / ADR-0014. |
| Bodoni Moda hero | Tipografia hero canônica. Pesos 400+500 via `next/font`. Weight adaptativo: 500 mobile (≤640px) / 400 desktop. Decisão S1.2 / ADR-0014. |
| Persona-Tipo Modelo Ella | Mecânica prompt-only (substitui Soul Character treinado da ADR-0012). Master + 4 sub-prompts. Cada geração independente via Nano Banana Pro 2K. Decisão S1.3 / ADR-0015. |
