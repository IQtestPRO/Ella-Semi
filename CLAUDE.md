...
- ❌ Usar **qualquer** skill `/gsd:*` (gsd:do, gsd:plan-phase, gsd:execute-phase, gsd:update, etc) neste projeto. Workflow GSD não é usado aqui — `CLAUDE.md` define skill flow próprio (Matt Pocock + Emil + minimalist-ui + Higgsfield) que cobre todo o trabalho. As skills `gsd:*` ficam disponíveis no setup global do Claude Code, mas **não devem ser invocadas dentro deste repo** (S2.0 / decisão Pak 2026-05-06).
- ❌ Renderizar peça meio-fotografada (1 ou 2 fotos no `product.fotos`). Schema Zod (`refine`) bloqueia: peça tem 0 fotos (camada placeholder ADR-0016) ou exatamente 3 fotos (catálogo Higgsfield ADR-0008). Estados intermediários são anti-padrão.
- ❌ Substituir `PlaceholderProductImage` por stock photo / banco de imagens / SVG comercial de terceiros. Componente é assinatura visual proprietária da marca (ADR-0016). Mudanças exigem ADR superando.
...
- **ADR-0016** — Camada Placeholder SVG transitória (S2.0). Componente `PlaceholderProductImage` com 9 silhuetas SVG inline por categoria + sparkle dourado + microtag "FOTO EM BREVE". Renderiza quando `product.fotos.length === 0`. Não substitui Higgsfield — peças trocam pra `<Image>` automaticamente quando ganham 3 fotos via S3.1. Schema Zod aceita 0 ou 3 fotos (atualização inline ADR-0008).
- **ADR-0017** — Schema amendment `maisVendido: boolean` (S2.0). Histórico de venda real loja física → seção "MAIS VENDIDOS" da home. Distinto de `destaqueHome` (curadoria editorial → "Favoritas da Ella"). Campo `destaque` proposto no plano S2.0 cortado (abstração sem dono). Naming `data/products.json` (en) preservado.
- **ADR-0018** — Hero da Marca via Persona-Tipo Higgsfield (ponte transitória, S2.0). Cinema Studio 3.0 + Nano Banana Pro 2K geram `hero-loop.mp4` + `hero-fallback.webp` até Ellen entregar foto-real autorizada. ADR registrada como **intencionalmente provisória** — substituição é parte do contrato; componente Hero não muda quando arquivo real chegar.
